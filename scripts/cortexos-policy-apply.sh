...
EOF
        ...
        # Pre-approve DefenseClaw device in openclaw paired.json
        if [ -f /usr/local/bin/cortexos-defenseclaw-pair ]; then
            /usr/local/bin/cortexos-defenseclaw-pair 2>&1 | grep -E "✅|❌|⚠️|Device ID|paired"
        fi
    else
