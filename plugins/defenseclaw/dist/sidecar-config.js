...
export function loadSidecarConfig() {
    ...
    try {
        ...
        const raw = yaml.load(readFileSync(cfgPath, "utf8"));
        if (raw && typeof raw === "object") {
            const gw = raw["gateway"];
            if (gw && typeof gw === "object") {
                if (typeof gw["host"] === "string" && gw["host"])
                    host = gw["host"];
                if (typeof gw["api_port"] === "number")
                    apiPort = gw["api_port"];
                if (typeof gw["token"] === "string" && gw["token"])
                    token = gw["token"];
                const tokenEnv = typeof gw["token_env"] === "string" && gw["token_env"]
                    ? gw["token_env"]
                    : DEFAULT_TOKEN_ENV;
...
export function _resetSidecarConfigCache() {
    cached = undefined;
}
